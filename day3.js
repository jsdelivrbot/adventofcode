...
var map = (function Map(){
	...
	self.AddPoint = function AddPoint () {
		if(!self.map[self.getCoordinates()]) {
			self.map[self.getCoordinates()] = 1; 
		} else {
			self.map[self.getCoordinates()] += 1;
		}
	};
	self.getVisitCount = function () {
	...
	for(i = 0; i < input.length; i++) {
		...
		switch(direction) { 
			...
			case '>':
				...
				break;
			default:
				console.log("error: didnt parse value")
				break;
		}
	}
	console.log(map.getVisitCount());
}

DeliverPresents(input); //2592